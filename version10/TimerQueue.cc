...
void TimerQueue::cancel(TimerId weakTimerptr)
{
    loop_->runInloop(std::bind(&TimerQueue::cancelInloop,this,weakTimerptr));
}

void TimerQueue::cancelInloop(TimerId weakTimerptr)
{
    loop_->assertInloopThread();
    std::shared_ptr<Timer> timer=weakTimerptr.value_.lock();
    if(timer)
    {
        TimerList::iterator it=timers_.find(Entry(timer->expiration(),timer));
        assert(it!=timers_.end());
        size_t n=timers_.erase(*it);
        assert(n==1);
    }
    else
    {
        LOG_DEBUG<<"Timer has been delete in other operation";
    }
}
...
std::vector<TimerQueue::Entry> TimerQueue::getExpired(Timestamp now)
{
    ...
    TimerList::iterator it=timers_.upper_bound(Entry(now,nullptr));
