...
void Connector::connecting(int sockfd)//get a channel and put into loop
{
    ...
    channel_->enableWriting();//add channel* into map and add channelfd into pollfds_
}
...
int Connector::removeAndResetChannel()//called in handleWrite
{
    ...
    loop_->queueInloop(std::bind(&Connector::resetChannel,this));//clear the shared_ptr
    return sockfd;
}
