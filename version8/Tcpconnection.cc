...
void Tcpconnection::connectEstablished()//put the connfd to the event loop, handle listenfd events
{
...
void Tcpconnection::sendInloop(const std::string& message)
{
    loop_->assertInloopThread();
    ssize_t nwrote=0;
    if(!channel_->isWriting()&&outputBuffer_.readableBytes()==0)//don,t exit data wait to send before
    {
        ...
        if(nwrote>=0)
        {
            ...
            else if(writeCompleteCallback_)//means all data write into sockfd
            {
                loop_->queueInloop(std::bind(writeCompleteCallback_,shared_from_this()));
            }
        }
